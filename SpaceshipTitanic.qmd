...
```{r}
svm_model <- svm_linear(mode = "classification", engine = "LiblineaR", cost = tune()) 

```

```{r}
svm_model_rec <- recipe(transported ~ ., data = train) %>%
  step_rm(passenger_id) %>%
  step_rm(name) %>%
  step_rm(cabin) %>%
  step_impute_median(all_numeric_predictors()) %>%
...
```{r}
svm_model_wf <- workflow() %>%
  add_model(svm_model) %>%
  add_recipe(svm_model_rec)
```


```{r}
svm_model_wf_results <- svm_model_wf %>% 
  tune_grid(
    resamples = train_folds,
    grid = 10,
    metrics = metric_set(accuracy, precision, recall)
...
svm_model_wf_results %>% 
  show_best(n = 10, metric = "recall")



svm_model_wf_results_lin <- svm_model_wf %>% 
  tune_bayes(
    resamples = train_folds,
    iter = 20,
    metrics = metric_set(recall),
    initial = svm_model_wf_results
  )

svm_model_wf_results_lin %>% 
  show_best(n = 10, metric = "recall")
```



This is a second model using the SVM Poly model.
```{r}
svm_model2 <- svm_poly(mode = "classification", cost = tune()) 


svm_model_rec2 <- recipe(transported ~ ., data = train) %>%
  step_rm(passenger_id) %>%
  step_rm(name) %>%
  step_rm(cabin) %>%
  step_impute_median(all_numeric_predictors()) %>%
...
svm_model_wf2 <- workflow() %>%
  add_model(svm_model2) %>%
  add_recipe(svm_model_rec2)


```

```{r}
svm_model_results2 <- svm_model_wf2 %>% 
tune_grid(
resamples = train_folds,
grid = 10,
metrics = metric_set(accuracy, precision, recall)
...
svm_model_results3 <- svm_model_wf2 %>% 
  tune_bayes(
    resamples = train_folds,
    iter = 20,
    metrics = metric_set(recall),
    initial = svm_model_results2
  )
...
## Conclusion
The first comparison I made was with the logistic regression model and a SVM LineaR model. The best mean of the logisic regression model was about 0.87 which is quite accurate. The SVM LineaR model was less accurate with a mean of about 0.78. After this model was made, I decided to create a second model with the same tuning but a different SVM model. Using the SVM Poly model type changed the original SVM Linear best mean from 0.77 to the SVM Poly best mean of 0.86 which is better than the Linear model but not the original Logistic regressison model.
